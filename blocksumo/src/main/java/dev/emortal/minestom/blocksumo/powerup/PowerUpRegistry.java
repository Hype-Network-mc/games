...
public final class PowerUpRegistry {
    ...
    public @NotNull WeightedList<PowerUp> findAllBySpawnLocation(@NotNull SpawnLocation spawnLocation) {
        List<WeightedList.Entry<PowerUp>> result = new ArrayList<>();
        for (PowerUp powerUp : this.registry.values()) {
            if (powerUp.getSpawnLocation() == spawnLocation) result.add(powerUp.getWeightedEntry());
        }
        return new WeightedList<>(result);
    }
