...
public class BetterEntityProjectile extends Entity {
    ...
    public void tick(long time) {
        ...
        Collection<EntityCollisionResult> collided = CollisionUtils.checkEntityCollisions(super.instance, super.getBoundingBox(), posBefore, diff, 3,
                entity -> entity instanceof Player player && player != this.shooter && player.getGameMode() != GameMode.SPECTATOR, result);

        for (EntityCollisionResult entityCollisionResult : collided) {
            if (entityCollisionResult.entity() instanceof Player player) {
                if (player != this.shooter) {
                    this.collidePlayer(entityCollisionResult.collisionPoint(), player);
                    return;
                }
            }
        }
