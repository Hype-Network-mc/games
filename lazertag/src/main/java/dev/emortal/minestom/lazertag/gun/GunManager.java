...
public final class GunManager {
    ...
    private void onReload(@NotNull PlayerInstanceEvent event) {
        ...
        Boolean reloading = item.getTag(Gun.RELOADING_TAG);
        if (reloading != null && reloading) return;
        if (item.getTag(Gun.AMMO_TAG) == heldGun.getItemInfo().ammo()) return;
    ...
    private void onTick(@NotNull PlayerTickEvent event) {
        ...
        Boolean reloading = player.getItemInMainHand().getTag(Gun.RELOADING_TAG);
        if (reloading != null && reloading) return;
    ...
    private void onItemUse(@NotNull PlayerUseItemEvent event) {
        ...
        Boolean reloading = item.getTag(Gun.RELOADING_TAG);
        if (reloading != null && reloading) return;
        if (item.getTag(Gun.COOLDOWN_TAG) > System.currentTimeMillis()) return;
        ...
        player.scheduler().submitTask(new Supplier<>() {
            ...
            public TaskSchedule get() {
                ...
                heldGun.getItemInfo().soundPlayer().play(event.getInstance(), player.getPosition());
